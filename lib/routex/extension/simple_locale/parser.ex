defmodule Routex.Extension.SimpleLocale.Parser do
  ...
  def extract_part(value, :region) do
    case value do
      <<_part::binary-size(2), sep, rest::binary>> when sep in @separator_chars -> rest
      <<_part::binary-size(3), sep, rest::binary>> when sep in @separator_chars -> rest
      _other -> nil
    end
  end
  ...
  defp parse_locale_with_quality(locale_string) do
    case String.split(locale_string, ";q=", parts: 2) do
      [locale, quality] -> parse_with_explicit_quality(locale, quality)
