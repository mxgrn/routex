defmodule Routex.Extension.SimpleLocale.Detect do
  ...
  @spec detect_locales(Plug.Conn.t() | map(), keyword(), keyword()) :: Types.locale_result()
  ...
  def normalize_locale_value(value, key) when key in [:region, :language] do
    case value do
      ...
      <<lang::binary-size(2), ?-, rest::binary>> -> if(key == :language, do: lang, else: rest)
      <<lang::binary-size(3), ?-, rest::binary>> -> if(key == :language, do: lang, else: rest)
      <<lang::binary-size(2), ?_, rest::binary>> -> if(key == :language, do: lang, else: rest)
      <<lang::binary-size(3), ?_, rest::binary>> -> if(key == :language, do: lang, else: rest)
      _other -> nil
    end
  end
