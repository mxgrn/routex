defmodule Routex.Extension.SimpleLocaleTest do
  ...
  describe "handle_params/4" do
    test "invokes the helper module and returns {:cont, socket}" do
      ...
      attrs = %{__backend__: DummyBackend, __helper_mod__: DummyHelper, locale: "en-US"}
  ...
  describe "plug/3" do
    test "invokes the helper module and adds accept_lang attr to the connection" do
  ...
  describe "parse accept language" do
    test "with language only fallbacks" do
      ...
      value = "*"
      expected = []
      [] = SimpleLocale.Parser.parse_accept_language(value)

      value = "en"
      expected = [%{language: "en", locale: "en", quality: 1.0, territory: nil, region: nil}]
      assert expected == SimpleLocale.Parser.parse_accept_language(value)

      value = "en-US"
      expected = [%{language: "en", locale: "en-US", quality: 1.0, territory: "US", region: "US"}]
      assert expected == SimpleLocale.Parser.parse_accept_language(value)
    end
  end
end
